	ORG $D000
; 1126 bytes ($D000 -  $D465)

MAGICTOP	EQU $B000
MAGIC_OBJBUF    EQU MAGICTOP+$16B6

; USR^Aエントリポイント: USR^A x, y, border*256+color
; FuzzyBASICのジャンプテーブル経由でSK_SK_SKを呼び、引数を取得する
PAINT_USR:
	CALL	$010C		; SK_SK_SK → HL=X, DE=Y, BC=border(B)|color(C)
; HL = X
; DE = Y
; BC = 色(C)と境界色(B)
; 境界色をFFHにすると境界色なし
...
; 1KBくらいのバッファのアドレス(MAGICと同時に使う前提でMAGICの3D Bufferを使う……)
BUFADR:
    DW  MAGIC_OBJBUF
